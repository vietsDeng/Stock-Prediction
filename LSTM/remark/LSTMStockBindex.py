...
class LSTMStockBindex():
    ...
    def get_train_data(self, file_path, time_step=10, rtype="train"):
        ...
        #处理成矩阵形式返回
        return np.array(data_temp["0"].tolist()).reshape(size , time_step * (8+20)), np.array(data_temp["1"].tolist()).reshape(size , 1)
    ...
    def get_train_softmax(self, file_path, time_step=10, rtype="train"):
        ...
        return np.array(data_temp["0"].tolist()).reshape(size , time_step * (8+20)), np.array(data_temp["1"].tolist()).reshape(size , 3)
